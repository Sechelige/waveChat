...
client.on('message', message => {
  ...
  if (message.content === '!cpu') {
    ...
    exec('cat /proc/cpuinfo | grep "model name" | uniq', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!cpucores') {
    const { exec } = require('child_process');
    exec('cat /proc/cpuinfo | grep "cpu cores" | uniq', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!cpumhz') {
    const { exec } = require('child_process');
    exec('cat /proc/cpuinfo | grep "cpu MHz" | uniq', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!cpuload') {
    const { exec } = require('child_process');
    exec('cat /proc/loadavg | awk \'{print $1}\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!memory') {
    const { exec } = require('child_process');
    exec('free -m | awk \'NR==2{printf "Memory Usage: %s/%sMB (%.2f%%)", $3,$2,$3*100/$2 }\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!disk') {
    const { exec } = require('child_process');
    exec('df -h | awk \'$NF=="/"{printf "Disk Usage: %d/%dGB (%s)", $3,$2,$5}\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!network') {
    const { exec } = require('child_process');
    exec('ip route get 1 | awk \'{print $NF;exit}\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!networkgateway') {
    const { exec } = require('child_process');
    exec('ip route show default | awk \'{print $3}\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!networkdns') {
    const { exec } = require('child_process');
    exec('cat /etc/resolv.conf | grep nameserver | awk \'{print $2}\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
  ...
  if (message.content === '!networkspeed') {
    const { exec } = require('child_process');
    exec('ethtool eth0 | grep Speed | awk \'{print $2}\'', (err, stdout, stderr) => {
      if (err) {
        console.error(err);
        return;
      }
